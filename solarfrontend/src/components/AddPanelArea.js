...
export const AddPanelArea = ({ selectedRoofPoints }) => {
  ...
  const rotatePanelsRight = () => {
    setRotationAngle((prev) => prev + 0.1); // 90 derece sağa dön
  };

  const rotatePanelsLeft = () => {
    setRotationAngle((prev) => prev - 0.1); // 90 derece sola dön
  };
  ...
  }, [rotationAngle]); // rotationAngle değiştiğinde panel düzenini güncelle
  ...
  const updatePanelLayout = (startPos, currentPos) => {
    ...
    const modelHeight = 8; // Model yüksekliği
